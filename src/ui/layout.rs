...
///
/// `collapse_sidebar` forces the list-only layout regardless of
/// width: the body becomes a single full-width region (returned as
/// `list`, with `preview`/`divider` `None`) so the focused embed can
/// own the whole screen. It's the same layout the narrow-terminal
/// path produces, so every downstream consumer that already handles
/// `preview: None` (the embed full-body render, `preview_rect`)
/// handles a collapsed sidebar for free.
pub fn compute(area: Rect, divider_col: Option<u16>, collapse_sidebar: bool) -> Layouts {
    ...
    if collapse_sidebar || area.width < PREVIEW_MIN_WIDTH {
        // Narrow terminal (mobile / small mosh) or sidebar manually
        // hidden while focused: list-only / full-body.
        return Layouts {
            list: body,
            preview: None,
            divider: None,
...
mod tests {
    ...
    fn narrow_terminal_hides_preview_and_gives_full_width_to_list() {
        let l = compute(rect(50, 24), None, false);
    ...
    fn exact_threshold_still_shows_preview_with_divider() {
        let l = compute(rect(PREVIEW_MIN_WIDTH, 24), None, false);
    ...
    fn just_below_threshold_is_list_only() {
        let l = compute(rect(PREVIEW_MIN_WIDTH - 1, 24), None, false);
        assert!(l.preview.is_none());
        assert!(l.divider.is_none());
    }
    ...
    fn wide_terminal_uses_default_38_percent_list_split() {
        let l = compute(rect(120, 30), None, false);
    ...
    fn user_divider_position_overrides_default() {
        let l = compute(rect(120, 30), Some(60), false);
    ...
    fn is_divider_col_matches_exact_column() {
        let l = compute(rect(120, 30), Some(50), false);
    ...
    fn is_divider_col_always_false_in_narrow_mode() {
        let l = compute(rect(40, 24), None, false);
        assert!(!is_divider_col(&l, 0));
        assert!(!is_divider_col(&l, 20));
    }

    #[test]
    fn collapse_sidebar_gives_full_body_even_on_wide_terminal() {
        // Wide enough to normally split, but collapse forces list-only.
        let l = compute(rect(120, 30), None, true);
        assert!(l.preview.is_none());
        assert!(l.divider.is_none());
        assert_eq!(l.list.width, 120);
        assert_eq!(l.list.height, 29); // body = total - 1 for statusbar
    }
}
