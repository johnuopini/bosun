...
const BANNERS: &[(&str, &[u8])] = &[
    ...
    // (5-row body, 1-row ascender/descender legs) rendered 2 cells per
    // pixel-column so the pixels read square, with a subtle dim band
    // behind the bottom 3 body rows. a-z; uppercase slots reuse the
    // same glyphs. Generated by `scripts/gen_opencode_tdf.py`.
    (
        "opencode",
