...
- **`opencode` banner font, now the default.** A custom lowercase pixel
  ...
  5-row body and 1-row ascender/descender legs, rendered two cells per
  pixel-column so the letters read square, with a subtle dim band behind
  the bottom three rows of each letter. It ships as a color TheDraw font
  (`themes/banners/opencode.tdf`) and is the new default for section
  headers and the empty-state splash; press `f` to cycle to the previous
  fonts (`eatmex`, `metalix`, …). Generated by, and tweakable via,
  `scripts/gen_opencode_tdf.py`.
